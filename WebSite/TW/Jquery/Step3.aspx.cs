...
namespace WebSite.Jquery
{
    public partial class Step3 : System.Web.UI.Page
    {
        ...
        protected List<string[]> ProjectArray()
        {
            ...
            workHazard.Add(new string[] { "出航及進入空域階段", "[初步危險分析法] 萬一在航路上無法與管制單位構聯", "輕微", "頻繁", "M13", string.Format(MOL_Btn, 1) });
            workHazard.Add(new string[] { "出航及進入空域階段", "[初步危險分析法] 萬一管制單位引導錯誤", "中等", "幾乎不", "L16", string.Format(MOL_Btn, 2) });
            workHazard.Add(new string[] { "空中階段", "[假設狀況法] 萬一雷達脫鎖", "中等", "很可能", "M9", string.Format(MOL_Btn, 3) });
            workHazard.Add(new string[] { "空中階段", "[假設狀況法] 萬一未檢查武器電門是否在模擬位置", "災難", "頻繁", "EH1", string.Format(MOL_Btn, 4) });
            workHazard.Add(new string[] { "起飛離場階段", "[初步危險分析法] 起飛離場時飛機故障", "輕微", "幾乎不", "L20", string.Format(MOL_Btn, 5) });
            workHazard.Add(new string[] { "起飛離場階段", "[萬一] 萬一滾行時發動機火警", "嚴重", "很少", "M11", string.Format(MOL_Btn, 6) });
