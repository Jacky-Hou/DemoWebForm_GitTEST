...
namespace WebSite.Jquery
{
    public partial class Step1 : System.Web.UI.Page
    {
        ...
        {
            var Del = "<input type='button' onclick='Del({0})' value='刪除'>";

            List<string[]> workHazard = new List<string[]>();
            workHazard.Add(new string[] { "出航及進入空域階段", "[初步危險分析法] 萬一在航路上無法與管制單位構聯", string.Format(Del, 1) });
            workHazard.Add(new string[] { "出航及進入空域階段", "[初步危險分析法] 萬一管制單位引導錯誤",string.Format(Del, 2) });
            workHazard.Add(new string[] { "空中階段", "[假設狀況法] 萬一雷達脫鎖", string.Format(Del, 3) });
            workHazard.Add(new string[] { "空中階段", "[假設狀況法] 萬一未檢查武器電門是否在模擬位置", string.Format(Del, 4) });
            workHazard.Add(new string[] { "起飛離場階段", "[初步危險分析法] 起飛離場時飛機故障",string.Format(Del, 5)  });
            workHazard.Add(new string[] { "起飛離場階段", "[假設狀況法] 萬一滾行時發動機火警", string.Format(Del, 6)});
