...
namespace WebSite.Jquery
{
    public partial class Step4 : System.Web.UI.Page
    {
        ...
        protected List<string[]> ProjectArray()
        {
            ...
            workHazard.Add(new string[] { "出航及進入空域階段", "[初步危險分析法] 萬一在航路上無法與管制單位構聯", "中等", "幾乎不", "L16", "已設定", "已設定", string.Format(INS_Btn,1), string.Format(COST_Btn,1), string.Format(DEC_Btn,1), "L14" });
            workHazard.Add(new string[] { "出航及進入空域階段", "[初步危險分析法] 萬一管制單位引導錯誤", "輕微", "偶而", "L18", "已設定", "已設定", string.Format(INS_Btn,2), string.Format(COST_Btn,2), string.Format(DEC_Btn,2), "H6" });
            workHazard.Add(new string[] { "空中階段", "[萬一] 萬一雷達脫鎖", "中等", "很可能", "M9", "已設定", "已設定", string.Format(INS_Btn,3), string.Format(COST_Btn,3), string.Format(DEC_Btn,3), "H5" });
            workHazard.Add(new string[] { "空中階段", "[萬一] 萬一未檢查武器電門是否在模擬位置", "災難", "頻繁", "EH1", "已設定", "已設定", string.Format(INS_Btn,4), string.Format(COST_Btn,4), string.Format(DEC_Btn,4), string.Format(RiskLevel, 4) });
            workHazard.Add(new string[] { "起飛離場階段", "[初步危險分析法] 起飛離場時飛機故障", "輕微", "幾乎不", "L20", "已設定", "已設定", string.Format(INS_Btn,5), string.Format(COST_Btn,5), string.Format(DEC_Btn,5), string.Format(RiskLevel, 5) });
            workHazard.Add(new string[] { "起飛離場階段", "[萬一] 萬一滾行時發動機火警", "嚴重", "很少", "M11", "已設定", "已設定", string.Format(INS_Btn,6), string.Format(COST_Btn,6), string.Format(DEC_Btn,6), string.Format(RiskLevel, 6) });
