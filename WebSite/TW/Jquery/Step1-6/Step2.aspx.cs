...
namespace WebSite.Jquery
{
    public partial class Step2 : System.Web.UI.Page
    {
        ...
        protected void InitDataGridView()
        {
            ...
            DT.Columns.Add("決定", typeof(string));
        ...
        string Amplitude = "<select id='sel_Amplitude_{0}' onchange='RiskLevel(this.id)'><option value ='1' > I.災難 </option><option value='2'>II.嚴重</option><option value ='3'> III.中等 </option><option value='4'> IV.輕微 </option></select>";
        string Probability = "<select id='sel_Probability_{0}' onchange='RiskLevel(this.id)'><option value = 'A'> A.頻繁 </option><option value='B'>B.很可能</option><option value = 'C' > C.偶而 </option><option value='D'> D.很少 </option><option value = 'E' > E.幾乎不可能 </ option ></ select >";
        ...
        string Edit = "<input id='Edit_{0}' type='button' onclick='Edit(this.id)' value='修改' />";
        ...
        protected List<string[]> ProjectArray()
        {
            ...
            workHazard.Add(new string[] { "出航及進入空域階段", "[初步危險分析法] 萬一在航路上無法與管制單位構聯", "輕微", "頻繁","M13", string.Format(Edit, 3) });
            workHazard.Add(new string[] { "出航及進入空域階段", "[初步危險分析法] 萬一管制單位引導錯誤", "中等", "幾乎不","L16", string.Format(Edit, 4) });
            workHazard.Add(new string[] { "空中階段", "[萬一] 萬一雷達脫鎖", "中等", "很可能","M9", string.Format(Edit, 6) });
            workHazard.Add(new string[] { "空中階段", "[萬一] 萬一未檢查武器電門是否在模擬位置",string.Format(Amplitude,7), string.Format(Probability,7), string.Format(RiskLevel, 7), string.Format(Confirm, 7) });
            workHazard.Add(new string[] { "起飛離場階段", "[初步危險分析法] 起飛離場時飛機故障", string.Format(Amplitude, 9), string.Format(Probability, 9), string.Format(RiskLevel, 9), string.Format(Confirm, 9) });
            workHazard.Add(new string[] { "起飛離場階段", "[萬一] 萬一滾行時發動機火警", string.Format(Amplitude, 10), string.Format(Probability, 10), string.Format(RiskLevel, 10), string.Format(Confirm, 10) });
