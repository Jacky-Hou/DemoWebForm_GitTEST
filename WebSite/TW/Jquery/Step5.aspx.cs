...
namespace WebSite.Jquery
{
    public partial class Step5 : System.Web.UI.Page
    {
        ...
        protected List<string[]> ProjectArray()
        {
            ...
            workHazard.Add(new string[] { "出航及進入空域階段", "[初步危險分析法] 萬一在航路上無法與管制單位構聯", "輕微", "頻繁", "M13", "已設定", "已設定", "已設定",  string.Format(RewardDiscipline, 1) });
            workHazard.Add(new string[] { "出航及進入空域階段", "[初步危險分析法] 萬一管制單位引導錯誤", "中等", "幾乎不", "L16", "已設定", "已設定", "已設定", string.Format(RewardDiscipline, 2) });
            workHazard.Add(new string[] { "空中階段", "[假設狀況法] 萬一雷達脫鎖", "中等", "很可能", "M9", "已設定", "已設定", "已設定", string.Format(RewardDiscipline, 3) });
            workHazard.Add(new string[] { "空中階段", "[假設狀況法] 萬一未檢查武器電門是否在模擬位置", "災難", "頻繁", "EH1", "已設定", "已設定", "已設定",  string.Format(RewardDiscipline, 4) });
            workHazard.Add(new string[] { "起飛離場階段", "[初步危險分析法] 起飛離場時飛機故障", "輕微", "幾乎不", "L20", "已設定", "已設定", "已設定", string.Format(RewardDiscipline, 5) });
            workHazard.Add(new string[] { "起飛離場階段", "[假設狀況法] 萬一滾行時發動機火警", "嚴重", "很少", "M11", "已設定", "已設定", "已設定", string.Format(RewardDiscipline, 6) });
