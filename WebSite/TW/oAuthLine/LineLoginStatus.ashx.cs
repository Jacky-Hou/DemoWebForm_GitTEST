...
{
    /// <summary>
    /// LineLoginStatus1 的摘要描述
    /// </summary>
    public class LineLoginStatus1 : IHttpHandler
    {

        public void ProcessRequest(HttpContext context)
        {
            string hidLoginSts = context.Request.Form["hidLoginSts"].ToString().Replace("/", "-");        //Line登入狀態
            string hidUID = context.Request.Form["hidUID"].ToString().Replace("/", "-");                  //LineID
            string hidUEmail = context.Request.Form["hidUEmail"].ToString().Replace("/", "-");            //Email 
            string hidUName = context.Request.Form["hidUName"].ToString().Replace("/", "-");              //LineName
