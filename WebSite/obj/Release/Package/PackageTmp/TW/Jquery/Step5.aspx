...
<head runat="server">
    ...
    <script>
        ...
        function RewardDiscipline_Model(id) {
            ...
            var Add = "<input id='Add_" + id + "' type='button' onclick='Add(this.id)' value='新增' />"
            ...
            var RDbody = $("#RewardDiscipline-body");
            RDbody.empty();

            var HeaderText = "<tr><td>工作項目</td><td>危險識別</td><td>獎勵</td><td>獎勵指標</td><td>懲戒</td><td>懲戒指標</td><td></td><td></td></tr>";
            RDbody.append(HeaderText);

            if (id == 1)
                RDbody.append("<tr><td>出航及進入空域階段</td><td>[初步危險分析法] 萬一在航路上無法與管制單位構聯</td><td>績優人員</td><td>公開表揚</td><td>未達標準人員</td><td>口頭警告</td><td>" + Edit + "</td><td>" + Del + "</td></tr>")
            else if (id == 2)
                RDbody.append("<tr><td>出航及進入空域階段</td><td>[初步危險分析法] 萬一管制單位引導錯誤</td><td>績優人員</td><td>個人獎金</td><td>未達標準人員</td><td>公開訓誡</td><td>" + Edit + "</td><td>" + Del + "</td></tr>")
            else if (id == 3)
                RDbody.append("<tr><td>空中階段</td><td>[萬一] 萬一雷達脫鎖</td><td>績優人員</td><td>放榮譽假</td><td>未達標準人員</td><td>依公司規定懲處</td><td>" + Edit + "</td><td>" + Del + "</td></tr>")
            else if (id == 4)
                RDbody.append("<tr>><td>空中階段</td><td>[萬一] 萬一未檢查武器電門是否在模擬位置</td><td>" + Reward + "</td><td>" + RewardContent + "</td><td>" + penalty + "</td><td>" + penaltyContent + "</td><td>" + Confirm + "</td><td>" + Del + "</td></tr>")
            else if (id == 5)
                RDbody.append("<tr><td>起飛離場階段</td><td>[初步危險分析法] 起飛離場時飛機故障</td><td>" + Reward + "</td><td>" + RewardContent + "</td><td>" + penalty + "</td><td>" + penaltyContent + "</td><td>" + Confirm + "</td><td>" + Del + "</td></tr>")
            else if (id == 6)
                RDbody.append("<tr><td>起飛離場階段</td><td>[萬一] 萬一滾行時發動機火警</td><td>" + Reward + "</td><td>" + RewardContent + "</td><td>" + penalty + "</td><td>" + penaltyContent + "</td><td>" + Confirm + "</td><td>" + Del + "</td></tr>")

            var div_RD = $("#div_RewardDiscipline");
            div_RD.empty();
            div_RD.append(Add);
...
<body>
    ...
    <form id="form5" action="Step3.aspx" method="post">
        ...
        <br />
        <label style="font-weight: bold;margin:0px 0px -5px 0px">執行風險控制</label>
        ...
        <div class="modal fade" id="RewardDiscipline" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-xl" role="document" style="width: 1000px">
                <div class="modal-content">
                    ...
                    <div id="div_RewardDiscipline" style="margin:0px 0px -5px 5px"></div>
