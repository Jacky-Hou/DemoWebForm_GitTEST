...
<head runat="server">
    ...
    <script>
        ...
        function INS_Model(id) {
            ...
            if (id == 1) {
                Istbody.append("<tr><td>出航及進入空域階段</td><td>[初步危險分析法] 萬一在航路上無法與管制單位構聯</td><td>轉移</td><td>轉移其他地點執行</td><td>" + ChkBox + "</td></tr>")
                Istbody.append("<tr><td>出航及進入空域階段</td><td>[初步危險分析法] 萬一在航路上無法與管制單位構聯</td><td>避免</td><td>迴避高風險</td><td>" + ChkBox + "</td></tr>")
            }
            else if (id == 2)
                ...
                Istbody.append("<tr><td>起飛離場階段</td><td>[初步危險分析法] 起飛離場時飛機故障</td><td>延後</td><td>安排下一班次</td><td>" + ChkBox + "</td></tr>")
            else if (id == 6)
        ...
        function COST_Model(id) {
            ...
            var HeaderText = "<tr><td>工作項目</td><td>危險識別</td><td>工具</td><td>作法說明</td><td>成本</td><td>效益</td><td>比率(效率/成本)</td><td>決策點選</td><td></td></tr>";
            Costbody.append(HeaderText);

            if (id == 1) {
                Costbody.append("<tr><td>出航及進入空域階段</td><td>[初步危險分析法] 萬一在航路上無法與管制單位構聯</td><td>轉移</td><td>轉移其他地點執行</td><td>4</td><td>3</td><td><label id='leb_" + id + "' >1.25</label></td><td>" + ChkBox + "</td><td>" + Edit + "</td></tr>")
                Costbody.append("<tr><td>出航及進入空域階段</td><td>[初步危險分析法] 萬一在航路上無法與管制單位構聯</td><td>避免</td><td>迴避高風險</td><td>4</td><td>3</td><td><label id='leb_" + id + "' >1.25</label></td><td>" + ChkBox + "</td><td>" + Edit + "</td></tr>")
            }else if (id == 2)
        ...
        function DEC_Model(id) {
            ...
            if (id == 1) {
                Costbody.append("<tr><td>[轉移] 轉移其他地點執行</td><td>0</td><td>0</td>><td>0</td><td>0</td><td>0</td><td>0</td><td>0</td><td>0</td><td>" + ChkBox + "</td><td>0</td></tr>")
                Costbody.append("<tr><td>[避免] 迴避高風險</td><td>0</td><td>0</td>><td>0</td><td>0</td><td>0</td><td>0</td><td>0</td><td>0</td><td>" + ChkBox + "</td><td></td>0</tr>")
            }else if (id == 2)
...
<body>
    ...
    <form id="form4" action="Step3.aspx" method="post">
        ...
        <br />
        <label style="font-weight: bold;margin:0px 0px -5px 0px">下達控制決策</label>
